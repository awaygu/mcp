...
const sample = {
  ...
  components: [
    ...
    { name: '直播间顶部栏', type: '导航栏', description: '主播头像、主播名称、人气值98.3K' },
  ],
  interactions: [
  ...
  states_detail: [
    ...
    { element: '场次卡片-黄金场', state: '置灰/禁用', condition: '积分<50' },
    { element: '随机匹配按钮', state: '置灰', condition: '未选场次和战兽' },
  ],
  key_info: ['原型文档页码：3/6', '积分规则：青铜场获胜1场+1积分', '活动时间：2026.10.01 - 2026.10.10'],
  visual_hierarchy: '',
};
...
check('保留「直播间顶部栏」（不误伤产品导航栏）', f.components.some((c) => c.name === '直播间顶部栏'));
...
check('不误伤日期 2026.10.01（点号分隔）', f.key_info.some((k) => k.includes('2026.10.01')));
check('结构化状态 3 → 2（滤除工具 UI 元素）', (f.states_detail || []).length === 2, `实际 ${(f.states_detail || []).length}`);
check(
  '保留「场次卡片-黄金场」状态三元组',
  (f.states_detail || []).some((s) => s.element === '场次卡片-黄金场' && s.condition === '积分<50')
);

// ─── 2. 数值冲突检测（原型.md 中真实存在的矛盾） ───────────────
console.log('\n【数值冲突检测】');
const domText = `
  白银场：20积分开启 黄金场：50积分开启 王者场：100积分开启
  场次开启门槛：青铜场获胜1场+1积分、白银场+3积分、黄金场+5积分、王者场+15积分
`;
const vlmTexts = [
  '积分规则：青铜场获胜1场+1积分、白银场+2积分、黄金场+3积分、王者场+5积分',
  '积分榜单说明：青铜场获胜1场+1积分、白银场+2积分',
];
...
check(
  '命中白银场 20 vs 2 或 3 vs 2',
  conflicts.some((c) => c.includes('白银场')),
  conflicts.find((c) => c.includes('白银场')) || ''
);
check(
  '未对一致项误报（青铜场均为 1）',
  !conflicts.some((c) => c.includes('青铜场'))
);

// 无冲突场景不应误报
const noConflict = detectNumericConflicts(domText, ['青铜场获胜1场+1积分']);
check('数值一致时不报冲突', noConflict.length === 0, `实际 ${noConflict.length} 条`);

// VLM 命中 DOM 中已有的合法值（白银场获胜 +3 积分在 DOM 中存在）→ 不报
const hitDomValue = detectNumericConflicts(domText, ['白银场获胜1场+3积分']);
...
const merged = mergePageResult({
  ...
  vlmSegments: [],
  imageAnalysis: [
    { src: 'u12.png', localPath: 'u12.png', summary: '积分规则说明图', texts: ['白银场+2积分', '黄金场+3积分', '王者场+5积分'] },
  ],
});
...
check(
  '图内冲突指明白银场 2 分',
  imgConflicts.some((c) => c.includes('白银场') && c.includes('2积分'))
);
...
const mergedClean = mergePageResult({
  ...
  vlmSegments: [],
  imageAnalysis: [
    { src: 'u12.png', localPath: 'u12.png', summary: '积分规则说明图', texts: ['白银场+3积分'] },
  ],
});
check(
