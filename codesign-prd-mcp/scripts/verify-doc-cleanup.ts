...
const page: MergedPage = {
  ...
  domText: [
    ...
    '余额：💎16255', // 独有 → 保留
    '余额：💎16255', // 重复 → 剔除
    '玩法流程：使用包裹礼物参与对应场次的斗兽 玩法入口：1.观众可见所有页面；', // 整段被控件块覆盖 → 剔除
    '—', // 纯符号 → 剔除
    '第三步', // 独有 → 保留
    '第三步', // 重复 → 剔除
    '胜', // 1 字行不做包含判定 → 保留
  ...
  blocks: [
    ...
    { i: 3, type: '矩形', name: '', lines: [CONGRATS], imgs: 0, rect: null },
    {
      ...
      type: '矩形',
      name: '',
      lines: ['玩法流程：使用包裹礼物参与对应场次的斗兽', '玩法入口：1.观众可见所有页面；'],
      imgs: 0,
      rect: null,
    },
    { i: 5, type: '矩形', name: '', lines: ['礼物1名称', '钻石'], imgs: 0, rect: null },
    { i: 6, type: '矩形', name: '', lines: ['礼物1名称', '钻石'], imgs: 0, rect: null },
...
check('多行规则块去重 ×2', doc.includes('- 礼物1名称 ×2'));
...
const TITLE = '装扮奖励列表(活动结束时钻石、蓝宝石手动发放、其他制品奖励自动发放）';
