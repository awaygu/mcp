...
- **分段截图**：超大页面自动网格分段滚动截图（垂直 × 水平，段间重叠 100px），确保宽图/长图内容完整不丢失
...
| 参数 | 默认值 | 说明 |
|---|---|---|
| 视口尺寸 | 1920x1080 | 每段截图尺寸 |
| 段间重叠 | 100px | 防止内容截断在边界 |
| 渲染等待 | 500ms | 滚动后等待渲染稳定 |
| 分段阈值 | 视口内 | 高宽均不超出视口才单张，任一轴超出即网格分段 |
| 最大段数 | 60 | 网格分段总数安全限制 |

**网格分段**：垂直与水平各自生成滚动位置序列（步进推进、末点强制贴边，避免末段被 clamp 产生重复画面），双轴组合逐段截图。宽流程图（如 5509x6050）会拆为 7 行 × 4 列的网格；截图前收集叶子元素矩形，**空白网格直接跳过**（如 28 格降至 24 段），既省段数也消除空白画面重复。
